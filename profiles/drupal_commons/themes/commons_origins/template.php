...
function commons_origins_preprocess_page(&$variables) {
  ...
  $variables['pre_header_menu'] = theme('grid_row', $variables['header_menu'], 'header-menu', 'full-width', $variables['grid_width']);
...
//	echo print_r($node);
        ...
        $variables['group_header_type'] = check_plain($node->field_group_type[0]['value']);
        $variables['group_header_location'] = check_plain($node->field_place[0]['value']);
        $variables['group_header_url'] = check_plain($node->field_url[0]['value']);
	$variables['group_header_members'] = check_plain($node->og_statistics['members_count']);
        $variables['group_header_posts'] = check_plain($node->og_statistics['posts_count']);
     }
      else {
